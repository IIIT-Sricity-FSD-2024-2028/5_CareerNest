...
const S = MOCK.submissions;

const setStatus = (card, status, label) => {
...
function toggleCard(h) { h.closest('.opp-card').classList.toggle('open'); }

function filterOpps(status,
    btn) {
        document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    btn.classList.add('active');
    document.querySelectorAll('.opp-card').forEach(c => c.style.display = (status === 'all' || c.dataset.status === status) ? '' : 'none');
}
...
function updateTabCounts() {
    ...
    document.querySelectorAll('.opp-card').forEach(c => {
        counts.all++; counts[c.dataset.status]++;
    });

    const tabs = document.querySelectorAll('.tab');
    ['all', 'pending', 'approved', 'published', 'rejected'].forEach((s,
        i) => tabs[i].textContent = `${s.charAt(0).toUpperCase() + s.slice(1)} (${counts[s]})`);
}

function approveCard(btn) {
    const card = btn.closest('.opp-card');
    setStatus(card, 'approved', 'Approved');
    card.querySelector('.action-bar').outerHTML = `<div class="action-bar"><button class="btn-publish" onclick="publishCard(this)"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg> Publish to Candidates</button></div>`;

    updateTabCounts();
}

function publishCard(btn) {
    const card = btn.closest('.opp-card');
    setStatus(card, 'published', 'Published');
    card.querySelector('.action-bar').outerHTML = `<div class="live-bar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg><span>Live — 0 candidates applied</span></div>`;

    updateTabCounts();
}

function openRejectModal(btn) {
    currentRejectCard = btn.closest('.opp-card');
    $('rejectRemarks').value = '';
    $('rejectRemarks').style.borderColor = '';
    let err = $('rejectError'); if(err) err.style.display = 'none';
...
function confirmRejection() {
    ...
    if (wordCount < 10) { 
        $('rejectRemarks').style.borderColor = '#ef4444'; 
        let err = $('rejectError'); if(err) err.style.display = 'block';
        return; 
    }
    let err = $('rejectError'); if(err) err.style.display = 'none';
    $('rejectRemarks').style.borderColor = '#e5e7eb';
    const card = currentRejectCard;
    if (!card) return;
    setStatus(card, 'rejected', 'Rejected');
    card.querySelector('.action-bar').outerHTML = `<div class="rejection-bar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#dc2626" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg><div><strong>Rejection Remarks</strong><p>${remarks}</p></div></div>`;

    updateTabCounts();
    closeRejectModal();
}

function buildOppCards(data) {
    return data.map(sub => {
        const statusLabel = sub.status.charAt(0).toUpperCase() + sub.status.slice(1);
        const typeLabel = sub.type === 'internship' ? 'Internship' : 'Placement';

        let actionHTML = '';
        if (sub.status === 'published') {
            actionHTML = `<div class="live-bar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg><span>Live — ${sub.applied} candidates applied</span></div>`;
        } else if (sub.status === 'approved') {
            actionHTML = `<div class="action-bar"><button class="btn-publish" onclick="publishCard(this)"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg> Publish to Candidates</button></div>`;
        } else if (sub.status === 'pending') {
            actionHTML = `<div class="action-bar">                 <button class="btn-approve" onclick="approveCard(this)"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg> Approve</button>                 <button class="btn-reject" onclick="openRejectModal(this)"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg> Reject with Remarks</button></div>`;
        } else if (sub.status === 'rejected') {
            const remark = sub.rejectionRemark || 'No remarks provided.';

            actionHTML = `<div class="rejection-bar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#dc2626" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg><div><strong>Rejection Remarks</strong><p>${remark}</p></div></div>`;
        }

        const skillsHTML = sub.skills.map(s => `<span class="skill">${s}</span>`).join('');


        return `<div class="opp-card" data-status="${sub.status}">             <div class="opp-header" onclick="toggleCard(this)">                 <div class="opp-left">                     <div class="opp-avatar" style="background:${sub.avatarBg}; color:#fff;">${sub.avatar}</div>                     <div class="opp-info">                         <div class="opp-title">${sub.title} <span class="type-badge ${sub.type}">${typeLabel}</span></div>                         <div class="opp-company">${sub.company} • ${sub.location} • ${sub.pkg}</div>                         <div class="opp-meta">Submitted by: ${sub.submitter} (${sub.company}) • ${sub.submitted} • Deadline: ${sub.deadline}</div>                     </div>                 </div>                 <div class="opp-right">                     <span class="status-badge ${sub.status}">${statusLabel}</span>                     <svg class="chevron" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>                 </div>             </div>             <div class="opp-details">                 <div class="details-grid">                     <div class="detail-left"><h4>Description</h4><p>${sub.desc}</p></div>                     <div class="detail-right">                         <h4>Eligibility</h4><p>Min CGPA: <strong>${sub.cgpa}</strong></p><p>Branches: <strong>${sub.branches}</strong></p><p>Backlogs: <strong>${sub.backlogs}</strong></p>                         <h4>Skills Required</h4><div class="skill-tags">${skillsHTML}</div>                         <h4>Rounds</h4><p class="rounds">${sub.rounds}</p>                     </div>                 </div>                 ${actionHTML}             </div>         </div>`;
    }).join('');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (oppList) {
        oppList.innerHTML = buildOppCards(S);
        updateTabCounts();
    }

    $('rejectModal')?.addEventListener('click',
        e => { if (e.target === $('rejectModal')) closeRejectModal(); });
});
